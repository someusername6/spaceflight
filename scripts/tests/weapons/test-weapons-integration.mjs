...
describe('Integration Tests', () => {
  it('Full flak explosion scenario entities created correctly', () => {
    ...
    assert.ok(projectiles.length === 1, 'Should have 1 projectile');
    assert.ok(projectiles[0] === flak, 'Projectile should be flak');
    ...
    assert.ok(enemies.length === 2, 'Should have 2 enemies');
  });

  it('Full nuke explosion scenario entities created correctly', () => {
    const world = createWorld();
    ...
    assert.ok(missiles.length === 1, 'Should have 1 missile');
