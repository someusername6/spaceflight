...
test('Lock accumulates when target is inside cone (directly ahead)', () => {
  // Target directly in front of player (0 degrees from forward)
  // Player faces -Z (identity quaternion), target at (0, 0, -1000)
  const { world, secondaryWeapons } = createTestSetup(new Vector3(0, 0, -1000));
...
test('Lock accumulates when target is at 30 degrees (inside 60 degree cone)', () => {
...
test('Lock does not accumulate when target is outside cone (90 degrees)', () => {
  // Target directly to the side (90 degrees from forward)
  // Player faces -Z, target at (1000, 0, 0) - perpendicular
  const { world, secondaryWeapons } = createTestSetup(new Vector3(1000, 0, 0));
...
test('Lock does not accumulate when target is behind (180 degrees)', () => {
...
test('Lock is lost when player turns away from target', () => {
  ...
  assert(lockBefore > 0.4, `Should have partial lock, got ${lockBefore}`);

  // Turn player 90 degrees - now facing +X instead of -Z
  playerTransform.rotation.setFromAxisAngle(new Vector3(0, 1, 0), Math.PI / 2);
...
test('Lock resumes when player turns back toward target', () => {
...
test('Lock works at exact cone boundary (60 degrees)', () => {
  ...
  assert(
    secondaryWeapons.lockProgress >= 0.4,
    `Lock should work at exact cone boundary (60°), got ${secondaryWeapons.lockProgress}`,
  );
});

test('Lock is lost just outside cone boundary (61 degrees)', () => {
  ...
  assert(
    secondaryWeapons.lockProgress === 0,
    `Lock should not work outside cone (61°), got ${secondaryWeapons.lockProgress}`,
  );
});

test('Lock works when target is above (inside vertical cone)', () => {
...
test('Full lock is lost when player turns away', () => {
