...
describe('Lobby Messages', () => {
  describe('Message type guards', () => {
    ...
    it('isChatMessage identifies ChatMessage messages', () => {
      const chatMsg = { type: GameMessageType.ChatMessage };
      const otherMsg = { type: GameMessageType.ReadyState };

      assert.strictEqual(isChatMessage(chatMsg), true);
      assert.strictEqual(isChatMessage(otherMsg), false);
    });
  ...
  describe('Message creators', () => {
    ...
    it('createChatMessage creates correct message', () => {
      const before = Date.now();
      const msg = createChatMessage('player-1', 'Hello!');
