...
describe('Permission Validation - Additional Cases', () => {
  describe('validateActionPermission', () => {
    ...
    it('should deny assignShip to another player', () => {
      const action = {
        type: 'assignShip',
        playerId: 'peer-2',
        shipId: 'ship-1',
      };
      const permissions = { ...DEFAULT_PERMISSION };
      const players = new Map([
        ['peer-1', createTestPlayerInfo('peer-1', 'Player1')],
        ['peer-2', createTestPlayerInfo('peer-2', 'Player2')],
      ]);
      ...
      assert.ok(error !== null);
      assert.ok(error.includes('yourself'));
    });

    it('should allow assignShip to self', () => {
      const action = {
        type: 'assignShip',
        playerId: 'peer-1',
        shipId: 'ship-1',
