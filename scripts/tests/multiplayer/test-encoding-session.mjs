...
describe('Protocol Encoding - Session Messages', () => {
  ...
  describe('CallsignChangeRequest message', () => {
    it('should round-trip encode/decode CallsignChangeRequest', () => {
      const msg = {
      ...
      const decoded = decodeMessage(encodeMessage(msg));
      assert.strictEqual(decoded.newCallsign, 'Iceman');
    });
  });

  describe('CallsignChanged message', () => {
    it('should round-trip encode/decode CallsignChanged', () => {
      const msg = {
        type: GameMessageType.CallsignChanged,
        playerId: 'peer-2',
        oldCallsign: 'Maverick',
        newCallsign: 'Iceman',
      };

      const decoded = decodeMessage(encodeMessage(msg));
      assert.strictEqual(decoded.playerId, 'peer-2');
      assert.strictEqual(decoded.oldCallsign, 'Maverick');
      assert.strictEqual(decoded.newCallsign, 'Iceman');
    });
  });
});
