...
describe('CampaignSyncManager', () => {
  ...
  describe('onCampaignUpdate callback', () => {
    ...
    it('should be called when Welcome message received (guest)', () => {
      const transport = createMockTransport();
      const router = createMessageRouter({
        transport,
        hostPeerId: 'host-peer',
      ...
      const welcomeMsg = {
        ...
        playerId: 'peer-1',
        campaignState: createTestCampaignState(),
        players: [createTestPlayerInfo('host-peer', 'Host')],
      };
      welcomeMsg.campaignState.credits = 88888;

      router.dispatch(encodeMessage(welcomeMsg), 'host-peer');

      assert.ok(updatedState !== null);
      assert.strictEqual(updatedState.credits, 88888);
    });
  });
