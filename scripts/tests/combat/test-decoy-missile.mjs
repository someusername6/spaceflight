...
describe('Decoy and Missile Systems', () => {
  console.log(`\n${'='.repeat(70)}`);
  ...
  for (const skill of SKILL_LEVELS) {
    ...
    decoyResults[skill] = {
      ...
      missilesFired:
        ...
        RUNS_PER_TEST,
      missilesHit:
        Object.values(agg.missilesHit).reduce((a, b) => a + b, 0) /
        RUNS_PER_TEST,
    };
  }

  console.log(
    'Skill      | Decoys | Success | Rate  | Missiles | Seduced | Hit',
  );
  console.log('-'.repeat(70));
  for (const skill of SKILL_LEVELS) {
  ...
  if (trackingTotal.fired > 0 && dumbfireTotal.fired > 0) {
    trackingRate = (trackingTotal.hit / trackingTotal.fired) * 100;
    dumbfireRate = (dumbfireTotal.hit / dumbfireTotal.fired) * 100;
    console.log(
  ...
  it('should have ace launch more decoys than rookie', () => {
    assert.ok(
      aceDecoys > rookieDecoys,
      `Ace (${aceDecoys.toFixed(1)}) should launch more decoys than Rookie (${rookieDecoys.toFixed(1)})`,
    );
  });

  it('should have missiles fire and hit', () => {
    assert.ok(
      trackingTotal.fired > 0 || dumbfireTotal.fired > 0,
      'No missiles were fired during test',
    );
  });
});
