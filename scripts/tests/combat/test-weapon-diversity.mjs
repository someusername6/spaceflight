...
describe('Weapon Damage Diversity', () => {
  console.log(`\n${'='.repeat(70)}`);
  ...
  it('should have good weapon diversity (no type >60%)', () => {
    assert.ok(
      maxPct <= 60,
      `Single weapon type exceeds 60%: ${maxPct.toFixed(1)}%`,
    );
  });

  it('should have sentinel with beam focus (>=40%)', () => {
    assert.ok(
      sentinelBeamPct >= 40,
      `Sentinel beam focus too low: ${sentinelBeamPct.toFixed(0)}%`,
    );
  });
});
