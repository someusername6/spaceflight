...
describe('Angular Velocity Calculation', () => {
  it('perpendicular movement at 500m', () => {
    ...
    assertApprox(
      ...
      0.2,
      0.001,
      `Expected 0.2 rad/s, got ${angularVel}`,
    );
  });

  it('radial movement produces near-zero angular velocity', () => {
    const shooterPos = new THREE.Vector3(0, 0, 0);
  ...
  it('closer range increases angular velocity', () => {
    const shooterPos = new THREE.Vector3(0, 0, 0);
  ...
  it('very close range returns 0', () => {
    ...
    assert.strictEqual(
      angularVel,
      0,
      `Expected 0 for close range, got ${angularVel}`,
    );
  });
});

describe('AI Profile Angular Factors', () => {
  it('angular factors decrease with skill', () => {
...
describe('Effective Max Error', () => {
  it('scales with angular velocity', () => {
  ...
  it('angular contribution is capped at 0.3 rad', () => {
    const prng = createPRNG(12345);
    const aimError = createAimError(prng, AI_PROFILES.regular);
