...
export function runStateTransitionTests() {
  ...
  test('Idle → Pursue when enemy detected', () => {
    ...
    const _player = createPlayerShip(
      game.world,
      'interceptor',
      new Vector3(0, 0, 0),
    );
  ...
  test('AI does not acquire dying entities as new targets', () => {
    ...
    assertEq(
      ai.state,
      AIState.Idle,
      'Should remain Idle with no valid targets',
    );
  });
}
