...
function randomPointOnSphere(rng: {
  random: () => number;
...
export function createStarGeometry(seed: string): THREE.BufferGeometry {
  ...
  const baseVertices = [
    [-STAR_QUAD_SIZE, -STAR_QUAD_SIZE, 0],
    [STAR_QUAD_SIZE, -STAR_QUAD_SIZE, 0],
    [STAR_QUAD_SIZE, STAR_QUAD_SIZE, 0],
    [-STAR_QUAD_SIZE, -STAR_QUAD_SIZE, 0],
    [STAR_QUAD_SIZE, STAR_QUAD_SIZE, 0],
  ...
  for (let i = 0; i < STAR_COUNT; i++) {
    ...
    const brightness = rng.random() ** STAR_BRIGHTNESS_POWER;
    const rot = quatRotBetweenVecs([0, 0, -1], pos);
    const baseIdx = i * 6 * 3;

    for (let v = 0; v < 6; v++) {
      const bv = baseVertices[v] as [number, number, number];
